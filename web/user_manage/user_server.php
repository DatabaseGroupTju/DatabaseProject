...
if(isset($order_column)){
    ...
    switch($i){
        case 0;$orderSql = " order by uid ".$order_dir;break;
...
//定义查询数据总记录数sql
$sumSql = "SELECT count(uid) as sum FROM USER";
//条件过滤后记录数 必要
$recordsFiltered = 0;
//表的总记录数 必要
$recordsTotal = 0;
...
for($j=0;$j<2;$j++){
    ...
    if(strlen($colSearchVal)>0 && $colSearchable == 'true' ){
        switch ($j) {
            case 0: $colSearchVal = "uid LIKE '%".$colSearchVal."%'";
                    array_push($search_arr,$colSearchVal);
                    break;
            case 1: $colSearchVal = "username LIKE '%".$colSearchVal."%'";
                    array_push($search_arr,$colSearchVal);
                    break;
            default:
                # code...
...
$zSearchSql = " username LIKE '%".$search."%'";
// 拼接搜索SQL
$sumSearchSql = '';
...
// query data
$sql='';
$totalResultSql = "SELECT * FROM USER";
$infos = array();
// 拼接最终SQL
